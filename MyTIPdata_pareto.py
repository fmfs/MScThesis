...
class ilpTIP(object):
    ...
    def run_big_data(self):
        ...
        duration = [3 for city in dict_cities]
        num_clusters = 5
        num_DestCluster = {i:5 for i in range(1,num_clusters+1)}
        StartSpan = '15/10/2019'
        ...
        if os.path.exists("/media/fmfs/Data1"):
            file_path = '/media/fmfs/Data1/Tese/befly/datafiles/mycsv_pareto.csv'
        else:
            file_path = '/home/fmfs/befly/datafiles/mycsv_pareto.csv'
        ...
        for iteration in range(100):
            self.weight_cost = 1 - iteration/100
            self.weight_time = 0 + iteration/100
            self.run_gftp()
    ...
    def run_gftp(self):
        ...
        obj1 = mdl.sum(mdl.sum(c[i,j,k]*x[i,j,k] for k in range(I[(i,j)])) for i,j in A_aux)
        obj2 = mdl.sum(mdl.sum(t[i,j,k]*x[i,j,k] for k in range(I[(i,j)])) for i,j in A_aux)

        mdl.set_multi_objective(sense = "min", exprs = [obj1, obj2], priorities = [1,1], weights = [weight_cost,weight_time], names = ["cost", "time"])
        ...
        #mdl.parameters.multiobjective.display.set(
        #    mdl.parameters.multiobjective.display.values.detailed)
        #ps1 = mdl.create_parameter_set()
        #ps2 = mdl.create_parameter_set()
        #ps1.add(mdl.parameters.dettimelimit, 50000)
        #ps2.add(mdl.parameters.dettimelimit, 25000)
        ...
        if os.path.exists("/media/fmfs/Data1"):
            file_path = '/media/fmfs/Data1/Tese/befly/datafiles/mycsv_pareto_solution.csv'
        else:
            file_path = '/home/fmfs/befly/datafiles/mycsv_pareto_solution.csv'
        with open(file_path, 'a', newline='') as f:
            fieldnames = ['number_of_clusters', 'number_of_cities', 'window_size', 'solve', 'cost', 'time', 'elapsed_time_cpu', 'elapsed_time_wall', 'pareto_cost_weight', 'pareto_time_weight']
            thewriter = csv.DictWriter(f, fieldnames=fieldnames)
            thewriter.writerow({'number_of_clusters' : n, 'number_of_cities' : m, 'window_size' : window_size, 'solve' : solution_dict, 'cost' : objective_cost, 'time': objective_time, 'elapsed_time_cpu' : elapsed_time_cpu, 'elapsed_time_wall' : elapsed_time_wall, 'pareto_cost_weight' : weight_cost, 'pareto_time_weight' : weight_time})
        mdl.end()
